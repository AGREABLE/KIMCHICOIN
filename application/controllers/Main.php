...
class main extends CI_Controller {
	...
	public function article() {
        ...
        if ( !$this->data['one'] ) {
            $this->data['one'] = $this->db->query( "SELECT *, {$this->langlib->DB_SELECT['BOARD']} FROM BOARD WHERE is_release = 'A' ORDER BY created_at DESC" )->row();
        }

        $this->data['prev'] = $this->db->query( "SELECT *, {$this->langlib->DB_SELECT['BOARD']} FROM BOARD WHERE is_release = 'A' AND created_at >= '{$this->data['one']->created_at}' 
                                                   AND external_key != '{$this->data['one']->external_key}' ORDER BY created_at ASC LIMIT 1" )->row();
        $this->data['next'] = $this->db->query( "SELECT *, {$this->langlib->DB_SELECT['BOARD']} FROM BOARD WHERE is_release = 'A' AND created_at <= '{$this->data['one']->created_at}' 
